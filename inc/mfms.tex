...
\label{sec:mfms}
    ...
    \Cref{fig:mfms-model} shows the general layout of the proposed method.
    ...
    % while the second stage employs measurements from 2.4 GHz radios to localize the agent within the coarse-estimation result attained in the former stage.
    ...
    \subsection{Stage 1: Grid-cell Level Localization }
    This section covers the first stage of localization of the proposed method where the propagation of the radio waves is approximated in grid-level granularity and a special path loss exponent term is derived.
      ...
      During the online phase, \gls{mfms} first localizes the agent's vicinity with a grid-level accuracy by employing a softmax classifier which is depicted in \Cref{fig:softmax}.
      ...
      \subsubsection{Spatially-coherent Path Loss Exponent Estimation}
        ...
        As can be seen in \Cref{eq:optimal-ple}, \gls{ldpl} model considers the environment as a whole while the path loss exponent, a scalar, is expected to approximate the propagation occurring\ldots
        ...
        % the path loss exponent is estimated in grid-level resolution.
    ...
    % The weights of each grid is learned with a softmax classifier, which is depicted in \Cref{fig:softmax}.
      \subsubsection{Anchor Node Selection}
