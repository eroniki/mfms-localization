\section{Radio Wave Propagation in Radiolocation and Function Approximation}
\label{sec:fundamentals}
    ...
    % Let $\bm{\Omega} = \{g_i | i : 1\ldots n_g \}$ be the localization environment which is divided int $n_g$ number of grid cells $g_i$ and $\bm{x}_i = \{ x, y, \theta \} \in \mathbb{R}^3$ be the pose of the agent to be localized.
    ...
    \Cref{fig:omega} illustrates the localization environment and the robot pose etc.
    ...
    % \subsection{Supervised Propagation Modelling}
    %     Supervised learning
    %
    % \subsection{Empirical Propagation Modelling}
